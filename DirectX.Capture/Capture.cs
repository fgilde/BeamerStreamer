...
namespace DirectX.Capture
{
    ...
    public class Capture
    {
        ...
        public Control PreviewWindow
        {
            ...
            {
                if(value != null)
                    previewFrameworkElement = null;
                assertStopped();
                derenderGraph();
                previewWindow = value;
                wantPreviewRendered = ((previewWindow != null || previewFrameworkElement != null) && (videoDevice != null));
                renderGraph();
                startPreviewIfNeeded();
            }
        }

        private System.Windows.FrameworkElement previewFrameworkElement;
        public System.Windows.FrameworkElement PreviewFrameworkElement
        {
            get { return previewFrameworkElement; }
            set
            {

                previewFrameworkElement = value;
                if(value != null)
                    PreviewWindow = null;
            }
        }

        private IntPtr GetPreviewHandle()
        {
            if (previewFrameworkElement != null)
            {
                HwndSource hwndSource = PresentationSource.FromVisual(previewFrameworkElement) as HwndSource;
                if (hwndSource != null)
                {
        ...
        protected void renderGraph()
        {
            ...
            var handle = GetPreviewHandle();
            // Render preview stream (only if necessary)
            if (wantPreviewRendered && !isPreviewRendered && handle != IntPtr.Zero)
            {
        ...
        private void RegisterResizeEvents()
        {
            ...
            if (previewFrameworkElement != null)
            {
                previewFrameworkElement.SizeChanged += onPreviewWindowResize;
                onPreviewWindowResize(this, null);
            }
        }
        ...
        {
            if (videoWindow != null && (previewWindow != null || previewFrameworkElement != null))
            {
                // Position video window in client rect of owner window
                Rectangle rc = new Rectangle();
                if (previewWindow != null)
                    rc = previewWindow.ClientRectangle;
                else if (previewFrameworkElement != null)                                    
                    rc = new Rectangle(0,0, (int)previewFrameworkElement.ActualWidth, (int)previewFrameworkElement.ActualHeight);                
